...
4. **Use a non-redundant information hierarchy.** Let rows, columns, axes, legends, and colorbars carry shared dimensions instead of restating them in every panel, and give each piece of information one appropriate home. Keep panel titles to the identifiers needed to distinguish neighboring panels; place shared context once at figure level only when needed; and use annotations mainly for panel-varying information relevant to the comparison. Put quantities, units, and category meanings in their normal axis, colorbar, or legend roles. Prefer concise labels when the surrounding structure establishes their meaning, and remove repeated context across figure titles, subtitles, panel titles, annotations, legends, and axis labels.
